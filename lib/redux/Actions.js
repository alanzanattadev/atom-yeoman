...
let creators = {
  ...
  runGenerator: function(namespace, name) {
    return (dispatch) => {
      let {taskRunner, modalPanel, modalYeomanView} = require('../yeoman');
      ProjectSelecter.selectProject({type: "file", name: "package.json"}, modalPanel, modalYeomanView.getElement())
        .then((cwd) => {
        ...
        .catch((e) => {
          console.log(`Working path not found : ${e && e.toString()}`);
          atom.notifications.addError(`Working path not found`, {detail: e && e.toString()});
        })
    }
  },
